...
export default function ChatMessagesPane({
  ...
  // Virtuoso needs a live reference to the scroll DOM element. Re-render once after mount
  // so it picks up scrollContainerRef.current (initially null on first render).
  ...
  const renderMessage = useCallback(
    (index: number) => {
      ...
      // Virtuoso renders each item in its own wrapper, so parent space-y-* is lost.
      // Reintroduce the vertical gap with margin-top (skip the very first item).
      return (
        <div className={index === 0 ? '' : 'mt-3 sm:mt-4'}>
          <MessageComponent
  ...
  const computeItemKey = useCallback(
    (index: number) => {
      const message = visibleMessages[index];
      return message ? getMessageKey(message) : `placeholder-${index}`;
    },
    [visibleMessages, getMessageKey],
  );
        ...
        <>
          ...
          {scrollParentEl && (
            <Virtuoso
              ...
              // Reasonable estimate for an unmeasured chat message (markdown + tool blocks
              // average ~250px). Without this, Virtuoso assumes ~50px per item and the
              // virtual list collapses, leaving no scrollable area above the initial position.
              defaultItemHeight={250}
              initialTopMostItemIndex={Math.max(0, visibleMessages.length - 1)}
              // Auto-follow streaming output only when user is at the bottom; if the user
              // has scrolled up, do NOT yank them back down.
