...
export default function ProviderSelectionEmptyState({
  ...
  const currentModelLabel = useMemo(() => {
    const found = CLAUDE_MODELS.OPTIONS.find(
      (o: { value: string; label: string }) => o.value === claudeModel,
    );
    return found?.label || claudeModel;
  }, [claudeModel]);
  ...
  const handleModelSelect = useCallback(
    ...
    [setProvider, setClaudeModel, textareaRef],
  );
                  ...
                  {PROVIDER_GROUPS.map((group, idx) => (
                    ...
                    >
                      {group.models.map((model) => {
                        const isSelected = provider === group.id && claudeModel === model.value;
                        return (
                          <CommandItem
