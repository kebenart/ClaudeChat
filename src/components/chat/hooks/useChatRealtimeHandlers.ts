...
export function useChatRealtimeHandlers({
  ...
  // Idle watchdog: if no WS message arrives for IDLE_TIMEOUT_MS while the UI still thinks
  // we're loading, force-clear the "回复中" indicator. Protects against missed `complete`
  // events (WS drop, provider-side bug, sessionId mismatch). Reset on every message below.
  ...
  useEffect(() => {
    ...
    // Any incoming message means the WS pipe is alive — refresh the idle watchdog so it
    // only fires after IDLE_TIMEOUT_MS of true silence.
    bumpIdleWatchdog();
