...
export default function McpServers({ selectedProvider, currentProjects }: McpServersProps) {
  ...
  const globalAddDescription = 'Add Global MCP Server writes one common stdio or HTTP server available to all scopes.';
  const providerAddDescription = `${providerButtonLabel} only changes ${providerName}.`;
  const globalModalDescription = 'Adds this MCP server globally. '
  ...
  return (
    <div className="space-y-4">
      ...
      {!IS_PLATFORM && <TeamMcpFeatureCard />}
