...
import { AppError, WORKSPACES_ROOT, validateWorkspacePath, resolveContextWindow } from '@/shared/utils.js';
...
import { liveSessionsService } from './services/recent-sessions.service.js';
...
import { getClaudeUsageLimits } from './services/claude-usage.service.js';
import { startEnabledPluginServers, stopAllPlugins, getPluginPort } from './utils/plugin-process-manager.js';
import { initializeDatabase, projectsDb } from './modules/database/index.js';
import { backfillRecentSessions } from './services/im-backfill.service.js';
import { pruneExpiredConversations } from './services/im-retention.service.js';
...
// IM hub API Routes (protected) — sync, messages, read cursors, conversation state
app.use('/api/im', authenticateToken, imRoutes);

// IM hook ingest Routes (NO JWT — terminal/IDE Claude sessions via settings hook).
// The router enforces its own loopback-only + optional shared-token gate.
app.use('/api/im-hook', imHookRoutes);

// Sessions API Routes (protected) — running + recent Claude sessions
app.use(
    '/api/sessions',
    authenticateToken,
    (() => {
        const rootDir = path.join(os.homedir(), '.claude', 'projects');
        const windowMin = Number(process.env.LIVE_SESSION_WINDOW_MIN) || 2;
        return createSessionsRouter({
            getRunningIds: getActiveClaudeSDKSessions,
            getLiveMeta: () => liveSessionsService.list({ rootDir, windowMin }),
            getProcessSessionIds: () => processSessionsService.list(rootDir),
            lookupSessionMeta: (id) => liveSessionsService.lookupById(rootDir, id),
            windowMin,
        });
    })()
);

// Usage rollup API Routes (protected) — 5h + weekly token spend
app.use(
    '/api/usage',
    authenticateToken,
    (() => {
        const rootDir = path.join(os.homedir(), '.claude', 'projects');
        return createUsageRouter({
            summarize: () => usageRollupService.summarize(rootDir),
            getClaudeLimits: (force) => getClaudeUsageLimits(force),
        });
    })()
);
...
app.get('/api/projects/:projectId/sessions/:sessionId/token-usage', authenticateToken, async (req, res) => {
    try {
        ...
        for (let i = lines.length - 1; i >= 0; i--) {
            try {
                ...
                if (entry.type === 'assistant' && entry.message?.usage) {
                    ...
                    modelId = entry.message?.model || null;
        ...
        // Resolve context window per model (CONTEXT_WINDOW env wins; [1m] and
        // large-context models like Opus 4.8 get 1M; else 200k).
...
async function startServer() {
    try {
        ...
        server.listen(SERVER_PORT, HOST, async () => {
            ...
            // IM ingestion is hook-driven, NOT watcher-driven. The jsonl file
            // watcher no longer feeds the IM hub: live IM messages come from the
            // SDK chat runtime (app sends) and the terminal Claude Code hook
            // (server/routes/im-hook.js, UserPromptSubmit + Stop) for terminal /
            // external sessions. Distilling on every jsonl append re-UPSERTed a
            // turn's bubble with growing content (the "still streaming" symptom)
            // and flooded clients with frames — so we deliberately do NOT register
            // a setSessionIndexedHook IM callback. The watcher still runs below to
            // drive the projects/file-tree UI (projects_updated); cold-start IM
            // history is seeded once by backfillRecentSessions.

            // Start watching the projects folder for changes
            await initializeSessionsWatcher();

            // One-time IM backfill of recent sessions so the conversation list
            // is populated on first launch (watcher only ingests live changes).
            void backfillRecentSessions().then((n) => {
                console.log(`IM backfill: distilled ${n} recent session(s) into the IM hub`);
                // Retention: prune conversations idle beyond the window now + hourly.
                const pruned = pruneExpiredConversations();
                if (pruned > 0) console.log(`IM retention: pruned ${pruned} stale conversation(s)`);
            }).catch((error) => {
                console.error('IM backfill failed', { error: error instanceof Error ? error.message : String(error) });
            });
            setInterval(() => {
                try {
                    const pruned = pruneExpiredConversations();
                    ...
                    console.error('IM retention sweep failed', { error: error instanceof Error ? error.message : String(error) });
                }
            }, 60 * 60 * 1000).unref();
        ...
        // Safety net: log and survive unhandled async errors instead of
        // letting Node terminate the process. Route handlers that miss a
        // try/catch should be fixed at the source, but this keeps the dev
        // loop alive while we find them.
