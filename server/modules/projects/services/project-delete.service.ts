...
import { imDb, projectsDb, sessionsDb } from '@/modules/database/index.js';
...
export async function deleteOrArchiveProject(projectId: string, force: boolean): Promise<void> {
  ...
  // Deleting a contact fully closes its IM conversations (messages + cursors).
  imDb.deleteConversationsByContact(row.project_path);
