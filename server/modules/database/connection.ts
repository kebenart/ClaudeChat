...
/**
 * Test-only: inject a Database instance as the shared connection so
 * repositories operate on it. Pass null to clear.
 */
export function __setConnectionForTests(db: Database.Database | null): void {
  if (process.env.NODE_ENV === 'production') {
    throw new Error('__setConnectionForTests must not be used in production');
  }
  instance = db;
}
