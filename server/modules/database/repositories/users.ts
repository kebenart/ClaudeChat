...
export const userDb = {
  ...
  /** Stores the AES-GCM sealed TOTP secret and bcrypt-hashed recovery code, and enables TOTP. */
  ...
  /** Clears all TOTP data for the user and disables TOTP. */
  ...
  /** Returns the current TOTP state for the user. */
  getTotpStatus(userId: number): {
    ...
    const row = db
      ...
      .get(userId) as TotpStatusRow | undefined;
    return {
