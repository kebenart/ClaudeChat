...
export const runMigrations = (db: Database) => {
  try {
    ...
    // `rev` is a globally-monotonic sync version bumped on insert AND in-place
    // update, so the /sync cursor re-delivers streaming content edits.
    const imMessagesColumns = getTableInfo(db, 'im_messages').map((column) => column.name);
    addColumnToTableIfNotExists(db, 'im_messages', imMessagesColumns, 'rev', 'INTEGER NOT NULL DEFAULT 0');
    db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_im_messages_conv_seq ON im_messages(conversation_id, seq)');
    db.exec('CREATE INDEX IF NOT EXISTS idx_im_messages_rev ON im_messages(rev)');
    db.exec('CREATE INDEX IF NOT EXISTS idx_im_conversations_activity ON im_conversations(last_activity_at)');

    // Server-synced per-conversation meta (note / fold) + path blacklist, so
    // pin/mute/note/fold/blacklist are shared across all clients instead of
    // living in each client's local storage.
    const imConvColumns = getTableInfo(db, 'im_conversations').map((column) => column.name);
    addColumnToTableIfNotExists(db, 'im_conversations', imConvColumns, 'note', 'TEXT');
    addColumnToTableIfNotExists(db, 'im_conversations', imConvColumns, 'is_folded', 'INTEGER NOT NULL DEFAULT 0');
    addColumnToTableIfNotExists(db, 'im_conversations', imConvColumns, 'is_deleted', 'INTEGER NOT NULL DEFAULT 0');
    db.exec(IM_BLACKLIST_TABLE_SCHEMA_SQL);
