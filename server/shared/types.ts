...
// ── IM hub contracts (consumed by both server/modules/* and server/services/*) ──
...
export type DistilledKind = 'text' | 'result' | 'question' | 'plan' | 'error' | 'choice' | 'image';
...
export interface DistilledMessage {
  ...
  content: string;
  createdAt: number;
  /** Folded tool activity for the gray collapsed bar. count = number of tool operations (tool_use blocks). rawRef* span the raw jsonl entry id range of this turn's tool activity, for the "view full record" viewer. */
