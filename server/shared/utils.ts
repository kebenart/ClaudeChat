...
/**
 * Resolve a model identifier to its effective context window (tokens).
 *
 * Order: CONTEXT_WINDOW env override → explicit `[1m]` variants → known
 * large-context models (Opus 4.8 defaults to 1M) → 200k (classic Claude).
 * The window is NOT fixed at 200k — newer models ship larger defaults; add a
 * pattern below, use the `[1m]` model suffix, or set CONTEXT_WINDOW.
 */
export function resolveContextWindow(model: string | null | undefined): number {
  const envOverride = Number.parseInt(process.env.CONTEXT_WINDOW ?? '', 10);
  if (Number.isFinite(envOverride) && envOverride > 0) return envOverride;
  if (typeof model === 'string') {
    if (/\[1m\]/i.test(model)) return 1_000_000;
    if (/opus-4-8/i.test(model)) return 1_000_000; // Opus 4.8 defaults to 1M context
  }
  return 200_000;
}
